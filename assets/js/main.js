document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!prefersReducedMotion.matches && supportsFinePointer.matches) {
    ...
    const animateSmoothScroll = () => {
      ...
      window.scrollTo(0, currentScrollY);
      scrollAnimationFrame = window.requestAnimationFrame(animateSmoothScroll);
    };
    ...
    const shouldIgnoreSmoothScroll = (event) => {
      if (
      ...
      if (
        ...
        event.target.closest("input, textarea, select, option, [contenteditable='true']")
      ) {
        return true;
      }

      return Math.abs(event.deltaY) < 0.01;
    };

    window.addEventListener(
      "wheel",
      (event) => {
        if (shouldIgnoreSmoothScroll(event)) {
          return;
        }

        event.preventDefault();
        targetScrollY = clamp(
          targetScrollY + normalizeWheelDelta(event),
          0,
          getMaxScrollY()
        );
    ...
    window.addEventListener("keydown", (event) => {
      ...
      window.setTimeout(syncSmoothScroll, 0);
    });

    window.addEventListener("resize", syncSmoothScroll);
    window.addEventListener("mousedown", syncSmoothScroll);
    window.addEventListener("touchstart", syncSmoothScroll, { passive: true });
  }
